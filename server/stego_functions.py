...
def saveDecoded(base, filename):
    if(len(base.shape) == 3):
        saveDecodedColor(base, filename)
    else:
        saveDecodedGray(base, filename)

def saveEncoded(base, hidden, filename):
    if(len(base.shape) == 3):
        saveEncodedColor(base, hidden, filename)
    else:
        saveEncodedGray(base, hidden, filename)
